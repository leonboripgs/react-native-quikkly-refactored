...
#define QUIKKLY_KEY_PHOTO_URI @"photoUri"
...
static void QuikklyInitialize() {
    ...
    dispatch_once(&onceToken, ^{
        ...
        QKQuikkly.blueprintFilename = @"metadallionBlueprint";
    });
}
...
RCT_EXPORT_METHOD(scanSingleImage:(NSDictionary *)options
                  resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
    NSString *uri = [options objectForKey:QUIKKLY_KEY_PHOTO_URI];
    QuikklyInitialize();
    
    NSURL *aLocalURL = [NSURL URLWithString: uri];
        
    UIImage *image=[UIImage imageWithContentsOfFile:aLocalURL.path];
    
    BOOL fileExists = [[NSFileManager defaultManager] fileExistsAtPath:aLocalURL.path];
    if (fileExists) {
        NSLog(@"requested file Exists");
    }
    
    if (image.imageAsset == nil) {
        NSLog(@"wrong filePath");
        resolve(@"image not exist");
        return;
    }
    @try {
        NSLog(@"%d",(int)CGImageGetWidth(image.CGImage));
        
        
        NSArray<QKScannable *> * results = [QKScannable detectInImage: image.CGImage];

        if (results.firstObject) {
            NSString* result = [NSString stringWithFormat:@"%@", @(results.firstObject.value)];

            NSLog(@"%@", result);
        }
        else {
            NSLog(@"invalid quikkly image");
        }
    }
    @catch(NSException *e){
        NSLog(@"error occured");
