...
public class QuikklyReactModule extends ReactContextBaseJavaModule implements ActivityEventListener {
    ...
    private static final String KEY_PHOTOURI = "photoUri";
    private static final String TEMPLATE_DEFAULT = "template0001style1";

    public static final String TAG = "quikkly-scanImage";
    ...
    @ReactMethod
    public void scanSingleImage(ReadableMap options, Promise promise) {
        String filePath = getString(options, KEY_PHOTOURI);

        try {
            Bitmap bitmap = MediaStore.Images.Media.getBitmap(this.getReactApplicationContext().getContentResolver() , Uri.parse(filePath));

            ScanResult result = Quikkly.getInstance().scanSingleImage(bitmap);
            if (result.tags.length > 0) {
                promise.resolve(result.tags[0].getData().toString());
            }
            else {
                promise.resolve(new String("not quikkly image"));
            }
        }
        catch (Exception e){
            e.printStackTrace();
            promise.reject("QuikklyUnkown", e.getMessage());
        }
    }
